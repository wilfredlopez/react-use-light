...
export class Locale {
  ...
  pressedKeys: string[];
  _appliedMacros: MacroI[];
  _keyMap: { [key: number]: string[] } = {};
  _killKeyCodes: number[];
  _macros: MacroI[];
  ...
  bindKeyCode(keyCode: string | number, keyNames: string | string[]) {
  ...
  bindMacro(
    keyComboStr: string,
    keyNames: string | string[] | Function | null,
  ) {
    if (typeof keyNames === "string") {
      keyNames = [keyNames];
    }
  ...
  getKeyNames(keyCode: number) {
    return this._keyMap[keyCode] || [];
  }

  setKillKey(keyCode: number | string) {
