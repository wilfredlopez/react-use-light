...
export class Keyboard {
  ...
  constructor(targetWindow?: any, targetElement?: any, targetPlatform?: any, targetUserAgent?: any) {
    this._targetWindow = targetWindow ?? null
    this._targetElement = targetElement ?? null
  ...
  bindRelease(keyComboStr, releaseHandler) {
    return this.bind(keyComboStr, null, releaseHandler, undefined)
  }
  ...
  _applyBindings(event: any = {}) {
    ...
    for (let i = 0; i < listenerGroups.length; i += 1) {
      const listeners: any = listenerGroups[i]
      const keyCombo: any = listeners[0].keyCombo
      ...
      if (
        ...
        for (let j = 0; j < listeners.length; j += 1) {
          ...
          if (this._appliedListeners.indexOf(listener as any) === -1) {
