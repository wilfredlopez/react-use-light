...
export type UseStateHistoryReturn<S> = [
  S,
  Dispatch<ResolvableHookState<S>>,
  HistoryState<S>,
];
...
export function useStateWithHistory<S = undefined>(): UseStateHistoryReturn<
  S | undefined
>;
    ...
    if (history.current.length) {
      ...
      if (history.current.length > capacity) {
        history.current = history.current.slice(
          history.current.length - capacity,
        );
      }
    } else {
  ...
  const historyState = useMemo(
    () => ({
      ...
      go: (position: number) => {
        ...
        innerSetState(() => {
          historyPosition.current = position < 0
