...
import useAudioControls from "../src/useAudioControls";

const setUp = (
  src: string = "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3",
  autoPlay: boolean = true,
) => renderHook(() => useAudioControls({ src, autoPlay }));
...
it("should init audio and utils", () => {
  ...
  const [{ audio, controls, state, ref }] = result.current;
  // if not production mode, it will show the error message, cause audio do not render
  expect(console.error).toHaveBeenCalledTimes(1);

  // Test the audio comp to React.ForwardRef
  expect(audio.type).toHaveProperty("$$typeof");
